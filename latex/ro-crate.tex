...
readable manner.
RO-Crate is based on \surl{Schema.org} annotations in JSON-LD, aiming to
establish best
...
Describo\footnote{\url{https://uts-eresearch.github.io/describo/}} \cite{describo} and
...
\cite{doi:10.1016/j.future.2011.08.004} is thus realized with the RO-Crate
model and serialized using Linked Data constructs within the RO-Crate
metadata file.
...
can extend and integrate RO-Crate in multiple standardized ways.
...
\textit{data entities}, which are further described by referencing {contextual
...
representation of a \textbf{data entity} as a byte sequence makes it
...
(top of Fig.~\ref{fig:conceptual}). This is a JSON-LD file that
describes the RO-Crate, its content and related metadata using Linked
Data. JSON-LD is a W3C standard RDF serialisation that has become
...
where their PID does not resolve to human-readable documentation.
...
relations from \surl{Schema.org} allow annotations to be more specific,
...
\surl{Schema.org}\footnote{\url{https://schema.org/}} \cite{doi:10.1145/2857274.2857276}
metadata as JSON-LD \cite{sporny_2014} (see Section~\ref{sec:implementation}), which leaves it open for implementers to include
additional metadata using other \surl{Schema.org} types and properties, or
...
the steep learning curve otherwise associated in Semantic Web
...
RO-Crate will, implicitly or explicitly, form a specialized \textbf{profile}
...
\surl{Schema.org} in a prescriptive form as discussed in Section~\ref{sec:conceptual}. These technical choices were made to cater for
...
extension \texttt{.json} instead of \texttt{.jsonld}, this is to emphasize the
...
referenced data or contextual entities, as exemplified by the data
...
RO-Crate. In this example, \texttt{about} of the CSV data entity reference the
...
excluded here for brevity. See complete example
\url
{https://www.researchobject.org/2021-packaging-research-artefacts-with-ro-crate/listing1/}.} this flattened profile of JSON-LD, each \texttt{{entity}} are directly
...
IRIs and applying the \texttt{@context} mapping to \surl{schema.org} terms.
\normalsize
...
enable various JSON-LD directives to cater for customized JSON
structures for translating to RDF.

RO-Crate reuses vocabulary terms and IRIs from \surl{Schema.org}, but provides
...
\surl{Schema.org} context means RO-Crate is also able to map in additional
...
Similarly, while the \surl{Schema.org} context
...
RO-Crate profile will use a high level \surl{Schema.org}
...
digitized over many years by researchers interviewing and recording
...
its own right, using \surl{Schema.org} as base vocabulary and extended with
...
Documentation Initiative (DDI), Dublin Core, DataCite, and \surl{Schema.org}.
...
This initially workflow-centric approach was further formalized as the
...
itself is recogized as a challenge \cite{doi:10.3233/978-1-61499-660-6-9}.
...
Metadata File.
\item[4.] By being based on Linked Data and \surl{schema.org} vocabulary, meaning
...
\textit{core profile} of \surl{schema.org} in JSON-LD, we cannot stress the
...
container provenance, which do not have a good match in \surl{Schema.org}.
...
discovering commonalities in emerging usage (e.g. additional \surl{schema.org}
...
matched well with existing \surl{schema.org} types but may benefit from other
...
\surl{schema.org} types like \url{http://schema.org/affiliation} and \url
...
A formalized RO-Crate in $\mathcal{L}_{\mathit{rocrate}}$ can be mapped to
...
{L}_{\textit{RDF}}$ using \surl{schema.org} as vocabulary:
%
...
{@context} maps to \surl{schema.org} IRIs, which, when resolved as Linked
...
Combining the above predicates and \surl{schema.org} mapping with rudimentary
...
data entity with \surl{schema.org} relation \url{http://schema.org/mentions}.
